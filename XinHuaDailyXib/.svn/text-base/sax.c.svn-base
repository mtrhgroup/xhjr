...
int stack_init (iksparser *prs);
int stack_expand (iksparser *prs, int len);
enum ikserror sax_core (iksparser *prs, char *buf, int len);
			...
			case C_ENTITY:
				if (';' == c) {
					...
					if (strcmp(prs->entity, "amp") == 0)
						...
						//add by william
						//sometimes the value is unicode chinese
						old += pos - old + 1;
						prs->context = C_CDATA;
						if (prs->cdataHook) {
							int j;
							char tmpEntity[10];
							tmpEntity[0] = '&';							
							for(j=1;j<prs->entpos+1;j++){
								tmpEntity[j] = prs->entity[j-1];
							}
							tmpEntity[prs->entpos+1] = ';';
							tmpEntity[prs->entpos+2] = '\0';
							err = prs->cdataHook (prs->user_data, &tmpEntity[0], prs->entpos+2);
							if (IKS_OK != err) return err;
						}
						break;
					}						
