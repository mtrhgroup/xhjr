...
static const CGFloat kWidthOfButtonMargin = 8.0f;
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(0, 0, textSize.width+kWidthOfButtonMargin*2, _shadowImage.size.height);
            button.selected = YES;
        }
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x-kWidthOfButtonMargin, 0, sender.frame.size.width+kWidthOfButtonMargin*2, _shadowImage.size.height)];
