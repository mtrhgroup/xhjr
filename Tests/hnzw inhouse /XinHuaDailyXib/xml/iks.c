...
size_t escape_size (char *src, size_t len);
char * my_strcat (char *dest, char *src, size_t len);
char * escape (char *dest, char *src, size_t len);
...
iks_find_cdata (iks *x, const char *name)
{
 ...
 iks *
iks_find_index (iks *x, const char *name, int index)
{
	iks *y;
	int i = 0;
	...
	y = IKS_TAG_CHILDREN (x);
	while (y) {
		if (IKS_TAG == y->type && IKS_TAG_NAME (y) && strcmp (IKS_TAG_NAME (y), name) == 0) {
			if (i == index) {
				return y;
			} else {
				i++;
 ...
 int
iks_numberOfTags (iks *x, const char *name)
{
	iks *y;
	int ret = 0;
 ...
 char *
iks_find_cdata_index (iks *x, const char *name, int index)
{
	iks *y;

	y = iks_find_index (x, name, index);
	if (!y) return NULL;
	y = IKS_TAG_CHILDREN (y);
	if (!y || IKS_CDATA != y->type) return NULL;
	return IKS_CDATA_CDATA (y);
...
{
     char* v = iks_find_attrib(x,name);
     return iks_unescape(iks_stack(x),v,iks_strlen(v));
}

 char* iks_find_attrib_name_index (iks *x, int index)
{
	if (x && IKS_TAG == x->type && IKS_TAG_ATTRIBS (x))
	{
		int count = 0;

		iks *y = IKS_TAG_ATTRIBS (x);
		while (y)
		{
			if(count==index)
				return IKS_ATTRIB_NAME (y);
			count++;
			y = y->next;
		}
	}
...
//char *
//iks_find_attrib_index (iks *x, const char *name, int index)
//{
//	iks *y;
//
//	if (!x) return NULL;
